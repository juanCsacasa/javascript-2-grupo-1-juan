...
const findChutemon = document.querySelector('form');
...
findChutemon.addEventListener('submit', event => {
  event.preventDefault();
  const query = document.getElementById('searchChutemon').value;
  fetch(`https://pokeapi.co/api/v2/pokemon/${query}`)
    ...
    .catch(error => {
      console.error('Ha ocurrido un error al buscar', error);
    });
});